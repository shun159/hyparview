defmodule Hyparview.PeerManager do
  ...
  def handle_INIT(:enter, _old_state, data) do
    ...
    {:keep_state_and_data, [{:state_timeout, @join_timeout, :join_timeout}]}
  end

  def handle_INIT(:state_timeout, :join_timeout, _data) do
    :repeat_state_and_data
  end

  def handle_INIT(:info, StartNeighbor, data) do
  ...
  def handle_JOINED(:info, StartNeighbor, data) do
  ...
  def handle_JOINED(:info, %Join{sender: sender} = join, data) do
    :ok = debug("JOIN request received from #{sender}")
    view = Join.handle(join, data.view)
    {:keep_state, %{data | view: view}}
  end

  def handle_JOINED(:info, %JoinAccepted{sender: sender} = join_accepted, data) do
    :ok = debug("JOIN accepted by #{sender}")
    view = JoinAccepted.handle(join_accepted, data.view)
    {:keep_state, %{data | view: view}}
  end

  def handle_JOINED(:info, %JoinFailed{sender: sender} = join_failed, data) do
    :ok = debug("JOIN rejected by #{sender}")
    _tref = JoinFailed.handle(join_failed, data.view)
    :keep_state_and_data
  end

  def handle_JOINED(:info, %ForwardJoin{sender: sender} = forward_join, data) do
    :ok = debug("FORWARDJOIN received from #{sender}")
    {_result, view} = ForwardJoin.handle(forward_join, data.view)
    {:keep_state, %{data | view: view}}
  end

  def handle_JOINED(:info, %Shuffle{sender: sender} = shuffle, data) do
    :ok = debug("SHUFFLE request received from #{sender}")
    view = Shuffle.handle(shuffle, data.view)
    {:keep_state, %{data | view: view}}
  end

  def handle_JOINED(:info, %ShuffleReply{sender: sender} = shuffle_reply, data) do
    :ok = debug("SHUFFLEREPLY received from #{sender}")
    view = ShuffleReply.handle(shuffle_reply, data.view)
    {:keep_state, %{data | view: view}}
  end

  def handle_JOINED(:info, %Neighbor{sender: sender} = neighbor, data) do
    :ok = debug("NEIGHBOR received from #{sender}")
    view = Neighbor.handle(neighbor, data.view)
    {:keep_state, %{data | view: view}}
  end

  def handle_JOINED(:info, %NeighborAccepted{sender: sender} = neighbor_accepted, data) do
    :ok = debug("NEIGHBOR ACCEPTED by #{sender}")
    view = NeighborAccepted.handle(neighbor_accepted, data.view)
    {:keep_state, %{data | view: view}}
  end

  def handle_JOINED(:info, %NeighborRejected{sender: sender} = neighbor_rejected, data) do
    :ok = debug("NEIGHBOR REJECTED by #{sender}")
    view = NeighborRejected.handle(neighbor_rejected, data.view)
    {:keep_state, %{data | view: view}}
  end

  def handle_JOINED(:info, %Disconnect{sender: sender} = disconnect, data) do
    :ok = debug("DISCONNECTED #{sender}")
    view = Disconnect.handle(disconnect, data.view)
    {:keep_state, %{data | view: view}}
  end

  def handle_JOINED(type, msg, _data) do
    :ok = debug(fn -> "Unhandled message received (type: #{type} msg: #{inspect(msg)})" end)
    :keep_state_and_data
  end
end
